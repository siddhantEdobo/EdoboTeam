    ...
    <nav className="navbar navbar-expand-lg edobo-white fixed-bottom shadow">
      <div className="container-fluid d-flex p-2 justify-content-between align-items-center me-4 ms-4">
        <ul className="list-unstyled d-flex justify-content-between w-100 h-100 m-0">
          ...
          >
            <div
              className={`mob-bottom-nav-link ${
                isActiveRoute(ROUTES_NAVIGATION.HOME) && "active-nav-link"
              }`}
            >
              <img
                className="footer-icon"
                src={
                  isActiveRoute(ROUTES_NAVIGATION.HOME) ? homeSelected : home
                }
                alt="Home"
              />
            </div>
          ...
          <li
            className="mob-bottom-nav"
            onClick={() => navigate(ROUTES_NAVIGATION.OFFERSCOUPON)}
          >
            <div
              className={`mob-bottom-nav-link ${
                isActiveRoute(ROUTES_NAVIGATION.OFFERSCOUPON) &&
                "active-nav-link"
              }`}
            >
              <img
          ...
          <li
            className="mob-bottom-nav"
            onClick={() => navigate(ROUTES_NAVIGATION.MOB_CATEGORY)}
          >
            <div
              className={`mob-bottom-nav-link ${
                isActiveRoute(ROUTES_NAVIGATION.MOB_CATEGORY) &&
                "active-nav-link"
              }`}
            >
              <img
                className="footer-icon-c"
                src={
                  isActiveRoute(ROUTES_NAVIGATION.MOB_CATEGORY)
                    ? categoriesSelected
                    : categories
          ...
          <li
            className="mob-bottom-nav"
            onClick={() => navigate(ROUTES_NAVIGATION.USER_ORDER)}
          >
            <div
              className={`mob-bottom-nav-link ${
                isActiveRoute(ROUTES_NAVIGATION.USER_ORDER) && "active-nav-link"
              }`}
            >
              <img
                className="footer-icon-c"
                src={
                  isActiveRoute(ROUTES_NAVIGATION.USER_ORDER)
                    ? myOrderSelected
                    : myorder
                }
                alt="My Orders"
