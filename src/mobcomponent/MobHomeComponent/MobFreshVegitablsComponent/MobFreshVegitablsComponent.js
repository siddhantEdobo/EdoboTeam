...
import "./MobFreshVegitablsComponent.css";
...
const MobFreshVegitablsComponent = (props) => {
  // const { EXPOLRE_LIST } = props;
  return (
        ...
        {EXPOLRE_LIST?.map((value) => {
          if (value?.large) {
            return (
              <div
                key={value?.id}
                className="ps-2 col-6 p-1 m-0 col-sm-4 mob-freshveg-image-large-container"
              >
                <img
            ...
            return (
              <div
                key={value?.id}
                className="ps-1 col-3 p-1 m-0 col-sm-2 mob-freshveg-image-container"
              >
                <img
                  className="w-100 h-100"
                  src={value?.url}
...
export default MobFreshVegitablsComponent;
