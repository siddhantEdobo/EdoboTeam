...
import { submitRegistration } from "../../redux/Slices/Auth/authSlice";

const RegistrationComponent = ({ token, mobile_no, onBack }) => {
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [gender, setGender] = useState("");
  const [age, setAge] = useState("");
  ...
  const handleContinue = () => {
    dispatch(submitRegistration({ name, email, gender, age, token }))
      .unwrap()
      .then(() => {
        navigate(ROUTES_NAVIGATION.HOME);
      });
  ...
  return (
    <div className="container mt-4">
      <div className="d-flex justify-content-center">
        <div className="card registration-component-card-container">
          <div className="mx-5 mt-2 otp-verification-btn-container flex-row">
            <FontAwesomeIcon
              icon={faArrowLeft}
              className="faicons-size text-white"
          ...
          <div className="registration-component-phone-email-text-container">
            <div className="d-flex gap-1 my-1">
              <img
                src={Images.loginPhoneNoImg}
                alt="email"
                className="mob-login-component-phone-email-img-container"
              />
              <div className="mt-1 fs-6 text-white">{mobile_no}</div>
            </div>
            <div className="input-group mt-3" style={{ maxWidth: "320px" }}>
              <span className="input-group-text">
                <img
