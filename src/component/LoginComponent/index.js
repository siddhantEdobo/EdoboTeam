...
import OTPVerificationComponent from "./OTPVerificationComponent";
import { Images } from "../../assets";
import { CustomModalError } from "../../common/CustomNotification";
import { ToastContainer } from "react-toastify";
import { CustomModalSuccess } from "../../common/CustomNotification/NotifySuccess";
import { useDispatch, useSelector } from "react-redux";
import { clearAuthState, generateOtp } from "../../redux/Slices/Auth/authSlice";
...
const LoginComponent = () => {
  ...
  // Redux state
  const { isLoading, otp, refId, successMessage, errorMessage } = useSelector(
    (state) => state.auth
  );
  ...
  useEffect(() => {
    if (successMessage) {
      ...
      const timer = setTimeout(() => {
        setShowSuccessModal(false);
        setShowOTPModal(true); // After success, show OTP modal
  ...
  const handleCloseSuccessModal = () => {
    setShowSuccessModal(false);
    setShowOTPModal(true); // Manually close and open OTP modal
  };
  ...
  return (
    ...
    >
                ...
                <div className="col-6">
                  <form>
                    ...
                    <div className="mt-2 login-component-continue-btn d-grid">
                      <button
                        className="btn"
                        onClick={(event) => handleOnLoginClick(event)}
                        type="submit"
                      >
                        Continue
                      </button>
                    ...
                    <div className="d-flex mt-3 ">
                      <div className="form-check">
                        <label className="form-check-label text-white cursor-pointer">
                          By continuing, I agree to the Terms of Use, Privacy
                          Policy & Cookie Policy.
                        </label>
                      </div>
      ...
      <CustomModalSuccess
        show={showSuccessModal}
        handleClose={handleCloseSuccessModal} // Handle manual close
        message={successMessage}
      />
